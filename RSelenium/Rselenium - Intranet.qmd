...
```{r}
remDr$navigate("http://intranet/") #https://hfrs.moh.go.tz
```
...
```{r}
webElem <- remDr$findElement(using = 'link text', "QPS")
...
```{r}
remDr$getPageSource() %>% purrr::flatten_chr() %>% read_html() %>% rvest::html_table()
```
